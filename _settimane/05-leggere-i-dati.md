...
Il file dei rendimenti contiene, per ogni giorno di Borsa del 2023, il rendimento giornaliero di quattro titoli di Piazza Affari: Ferrari, Enel, Intesa e Unicredit. Un rendimento dello 0,01 significa che il titolo ha guadagnato l'1 % rispetto al giorno prima. È il dataset del pilastro 1 su cui impariamo a **riassumere**: una colonna di 250 numeri in due o tre.
...
La **media** è la somma diviso il numero: il rendimento «tipico» se ogni giorno fosse uguale. La **mediana** è il valore che sta a metà una volta ordinati i giorni: metà dei giorni ha fatto peggio, metà meglio. Sono uguali quando la distribuzione è simmetrica; si allontanano quando ci sono pochi giorni estremi, e la media viene trascinata da quelli (è l'aspettativa di vita del Burundi, settimana 1). I **quantili** generalizzano la mediana: il quantile 0,25 lascia sotto di sé un quarto dei giorni.
...
> **Attenzione** `sd()` e `mean()` con un `NA` restituiscono `NA`: vale il `na.rm = TRUE` della settimana scorsa. Ma togliere gli `NA` non è neutro: se i giorni mancanti sono proprio quelli di crisi (dati non scaricati perché il mercato era sospeso), la deviazione standard è sottostimata. Prima di togliere, chiedersi perché mancano.
...
Il **grafico a barre** conta o confronta categorie. Sull'ecommerce della settimana scorsa, il fatturato per categoria:
...
Ogni riga di `case_when` è un `else if`: condizione a sinistra della tilde, valore a destra. Come nell'`if` della settimana 2, vince la **prima** condizione vera dall'alto: un cliente 4-4-4 è Gold e non Fedele, perché Gold viene prima. La riga `TRUE ~ "Altri"` è l'`else` finale.
...
## Il progetto di metà corso

Da questa settimana ogni gruppo lavora sul proprio dataset: una domanda di business, un'analisi descrittiva con dplyr e tre grafici con ggplot2, più un albero validato che impareremo nella settimana 6. Consegna: uno script R, tre grafici e una pagina di risultati.

## Per approfondire

I verbi di dplyr su cui si regge ogni riga di questa settimana sono nella [settimana 4](04-dati). Nella [settimana 6](06-software-2-0) le regole dell'autovelox, che finora abbiamo scritto noi, le farà trovare un algoritmo ai dati.
